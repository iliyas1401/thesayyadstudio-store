...
function CollectionsPage() {
  ...
  const filtered =
    filter === "ALL"
      ? products
      : products.filter((p) => p.category === filter || p.collection === filter);
            ...
            return (
              <div key={p.id} className="bg-white group">
                ...
                <div className="grid grid-cols-2 gap-2 mt-4">
                  <button
                    onClick={() => handleAdd(p)}
                    className="border border-black py-3 text-[10px] font-bold uppercase tracking-widest cursor-pointer hover:bg-black hover:text-white transition-colors"
                  >
                    Add to Cart
                  </button>
                  <button
                    onClick={() => handleBuy(p)}
                    className="bg-black text-white py-3 text-[10px] font-bold uppercase tracking-widest cursor-pointer hover:bg-gray-900 transition-colors"
                  >
                    Buy Now
                  </button>
                </div>
