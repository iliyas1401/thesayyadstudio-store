      ...
      <section className="relative min-h-screen bg-bone overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-6 lg:px-10 pt-28 lg:pt-32 pb-16 grid lg:grid-cols-12 gap-10 items-center min-h-screen">
          <div className="lg:col-span-5 text-center lg:text-left">
            <h1 className="font-display text-5xl lg:text-7xl leading-[1] tracking-tight text-foreground">Elevate Your<br /><em className="font-light text-primary">Everyday Style.</em></h1>
            <div className="mt-10"><a href="#collections" className="inline-flex bg-primary text-primary-foreground px-7 py-4 text-xs font-semibold uppercase tracking-[0.18em] rounded-full cursor-pointer">Shop The Collection</a></div>
          </div>
          <div className="lg:col-span-7 relative"><img src={heroTees} alt="Premium tees" className="w-full h-auto object-cover shadow-2xl rounded-md" /></div>
        </div>
      </section>
      ...
      <section id="collections" className="py-24">
        <div className="mx-auto max-w-7xl px-6">
          <div className="flex flex-wrap justify-center gap-4 mb-14">
            {categories.map(cat => (
              <button key={cat} onClick={() => setActiveCategory(cat)} className={`px-5 py-2 text-xs font-bold uppercase rounded-full border cursor-pointer ${activeCategory === cat ? 'bg-primary text-primary-foreground' : 'border-border'}`}>{cat}</button>
            ))}
          </div>
